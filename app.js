...
// Garante workerSrc (LEGACY) e ativa fallback sem worker se falhar
function ensurePdfWorker(){
  if (window.pdfjsLib && window.pdfjsLib.GlobalWorkerOptions) {
    window.pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@4.8.69/legacy/build/pdf.worker.min.js';
  }
}
ensurePdfWorker();
...
async function readPdfText(file, tryNoWorker=false){
  const data=await file.arrayBuffer();
  if (tryNoWorker && window.pdfjsLib) window.pdfjsLib.disableWorker = true;
...
async function parseInvoicePDF(file){
  ...
  let raw='';
  try{
    raw = await readPdfText(file, false);
  }catch(e1){
    console.warn('Worker falhou, tentando sem worker...', e1);
    ensurePdfWorker();
    try{
      raw = await readPdfText(file, true); // sem worker
    }catch(e2){
      console.error('Falha geral no PDF', e2);
      throw e2;
    }
  }
  let text=raw.replace(/\\s{2,}/g,' ').replace(/\\u00A0/g,' ').trim();
...
formFatura.addEventListener('submit', async (e)=>{
  ...
  try{
    ...
    previewList.innerHTML='<div class=\"sub\">Erro ao ler o PDF. Fallback ativado: se persistir, exporte CSV/OFX no app do banco e importe aqui (próxima versão).</div>';
    importBuffer=[];
  }
});
